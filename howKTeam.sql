...
FROM dbo.GIAOVIEN a
INNER JOIN dbo.BOMON b ON a.MABM=b.MABM
...
FROM dbo.KHOA
--1c
SELECT b.MAGV AS MA_GV, b.HOTEN AS TEN_GV, a.TEN AS TEN_NGUOI_THAN
FROM dbo.NGUOITHAN a
INNER JOIN dbo.GIAOVIEN b ON a.MAGV=b.MAGV
--1d
SELECT a.MAGV AS MA_GV, a.HOTEN AS TEN_GV, c.TENKHOA AS TEN_KHOA
FROM dbo.GIAOVIEN a
INNER JOIN dbo.BOMON b ON a.MABM=b.MABM
...
FROM dbo.GIAOVIEN a
WHERE YEAR(GETDATE())-YEAR(a.NGSINH) < 40
...
--Lấy ra tất cả thông tin của giảng viên có chức vụ là trưởng bộ môn
SELECT a.*
FROM dbo.GIAOVIEN a
INNER JOIN dbo.BOMON b ON a.MABM=b.MABM
WHERE b.TRUONGBM=a.MAGV

--Xuất ra tên giáo viên và tên đề tài giáo viên đó tham gia
SELECT DISTINCT b.HOTEN AS TEN_GV, c.TENDT AS TEN_DETAI
FROM dbo.THAMGIADT a
INNER JOIN dbo.GIAOVIEN b ON a.MAGV=b.MAGV
INNER JOIN dbo.DETAI c ON a.MADT=c.MADT 

--Đếm số lượng người thân của giáo viên của mã là 007
SELECT COUNT(*) AS SL_NGUOITHAN
FROM dbo.NGUOITHAN
WHERE MAGV='007'
...
WHERE a.GVQLCM=b.MAGV

--Xuất ra số lượng giáo viên của khoa CNTT
SELECT COUNT(*) AS SL_GV
FROM dbo.GIAOVIEN a, dbo.BOMON b, dbo.KHOA c
WHERE a.MABM=b.MABM AND b.MAKHOA=c.MAKHOA AND c.MAKHOA='CNTT'

--Xuất ra thông tin giáo viên và đề tài người đó tham gia khi KQ là đạt
